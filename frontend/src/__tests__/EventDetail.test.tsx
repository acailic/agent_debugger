...
describe('EventDetail', () => {
  ...
  it('surfaces repair attempt context and prior repairs', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Increase timeout and retry the request' })).toBeInTheDocument()
    expect(screen.getByText('Checks still flaky under load')).toBeInTheDocument()
    expect(screen.getByText(/Prior attempts 1/)).toBeInTheDocument()
    expect(screen.getByText(/Prior failed 1/)).toBeInTheDocument()
    expect(screen.getByText('Prior Repair Attempts')).toBeInTheDocument()
    expect(screen.getAllByText('Retry the request once')).toHaveLength(2)
    expect(screen.getByText(/\+ timeout: 60\s+\+ retry: 2/s)).toBeInTheDocument()
  })
})
