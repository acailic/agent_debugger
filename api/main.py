...
def create_app() -> FastAPI:
    ...
    @app.post("/api/auth/keys", response_model=CreateKeyResponse, status_code=201)
    ...
    @app.get("/api/auth/keys", response_model=list[KeyListItem])
    async def list_keys(
        tenant_id: str = Depends(get_tenant_id),
        db: AsyncSession = Depends(get_db_session),
    ):
    ...
    @app.delete("/api/auth/keys/{key_id}", status_code=204)
