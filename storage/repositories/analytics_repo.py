"""Analytics repository for local user analytics data access.

This repository provides a clean abstraction over the SQLite analytics database,
encapsulating all direct database operations. It maintains the same public API
as the original analytics_db module for backward compatibility.
"""
...
class AnalyticsRepository:
    ...
    Provides synchronous SQLite operations for tracking user debugging
    efficiency metrics. Uses a separate analytics.db file from the main
    database with fire-and-forget writes that don't block callers.
    """
    ...
    def __init__(self, db_path: Path | None = None):
        ...
        Args:
            db_path: Path to analytics.db. If None, uses default ./data/analytics.db
        """
        self._db_path = db_path or (Path.cwd() / "data" / "analytics.db")

    @property
    def db_path(self) -> Path:
        """Return the path to the analytics database file."""
        return self._db_path

    def ensure_schema(self) -> None:
        """Ensure the analytics database schema exists.
    ...
    def get_daily_breakdown(self, days: int = 14) -> list[dict[str, Any]]:
