...
def upgrade() -> None:
    # Get connection and inspector for idempotent operations
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    sessions_columns = {col["name"] for col in inspector.get_columns("sessions")}
    tables = set(inspector.get_table_names())

    # Add retention columns to sessions table (idempotent)
    if "retention_tier" not in sessions_columns:
        op.add_column(
            "sessions",
    ...
    if "failure_fingerprint_primary" not in sessions_columns:
        op.add_column(
            "sessions",
    ...
    if "cluster_representative" not in sessions_columns:
        op.add_column(
            "sessions",
    ...
    if "cluster_id" not in sessions_columns:
        op.add_column(
            "sessions",
            sa.Column("cluster_id", sa.String(36), nullable=True),
        )

    # Create failure_clusters table (idempotent)
    if "failure_clusters" not in tables:
        op.create_table(
            "failure_clusters",
            ...
            sa.Column("avg_severity", sa.Float(), nullable=False, server_default="0.0"),
        )
    ...
    # Create anomaly_alerts table (idempotent)
    if "anomaly_alerts" not in tables:
        op.create_table(
            "anomaly_alerts",
            sa.Column("id", sa.String(36), primary_key=True),
            sa.Column("tenant_id", sa.String(64), nullable=False, server_default="local"),
            ...
            sa.Column("detection_source", sa.String(32), nullable=False),
