...
class TestDeterministicRestoreHooks:
    ...
    async def test_restore_context_calls_hook(self):
        ...
        """LangChain restore hook should be available."""
        try:
            from agent_debugger_sdk.checkpoints import (
                LANGCHAIN_RESTORE_HOOK,
                RESTORE_HOOK_REGISTRY,
            )

            assert "langchain" in RESTORE_HOOK_REGISTRY or LANGCHAIN_RESTORE_HOOK is not None
        except ImportError:
            pytest.skip("LangChain restore hook not yet implemented")
