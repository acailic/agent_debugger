...
| Q14 | DONE (first slice) 2026-09-21 | Deterministic sanitized incident bundles (content-hashed, versioned), assertion runner through the current engine, baseline/candidate comparison with the shared-data gate, CLI; 13 tests incl. tamper/determinism/regression-exactness (`c61c838`). Multi-baseline suites, scenario-family assertions and CI wiring remain | Q05 ✓, Q08 ✓ | ✓(slice) |
| Q15 | DONE 2026-09-21 | Real-framework tests for langchain-core 1.6.3 + pydantic-ai 2.46.0 (16 tests, skip cleanly elsewhere), LangChain handler fixed for >=1.0 drift, honest capability matrix published at docs/adapters/capability-matrix.md (`c8d11b3`); CI job added | Q09 ✓ | ✓ |
...
| [MAST](papers/why-do-multi-agent-llm-systems-fail-mast.md) | DONE typing (4 modes + unmapped) | `mast_mode`/`mast_category` on first-bad-decision detail and failures; 6 firing rows + honest unmapped | Fixture suite per firing mode as a regression-lab gate; inter-agent modes when multi-agent capture lands; W03/W09 |
...
| [Weiser slicing](papers/weiser-program-slicing.md) | DONE slice functions + API | backward/forward slice + damage-radius over the evidence graph; GET /slices, GET /damage-radius | Record dependence edges as first-class events; one-click UI views; W03/W08 |
| [Tarantula](papers/tarantula-test-information-fault-localization.md) | DONE spectrum | Ochiai suspiciousness over bundle runs (`spectrum.top_suspects`) | Wire top-suspects pre-ranking into first-bad-decision candidates; spectrum-shift CI gate; W05 |
...
| [τ-bench pass^k](papers/tau-bench-pass-k-reliability.md) | DONE metric | Worst-of-k `reliability` (k, passes, pass_hat_k) on bundle reports | Re-run bundles k times in CI and gate on pass_hat_k; W05 |
