...
| Main CI | **RECOVERY IN PROGRESS** | First post-fix matrix on `739fecc`: Python 3.10/3.11/3.12 all green (the #324 fix confirmed in real CI, including the full xdist suite, coverage gate, contract check and frontend steps). The overall run still failed on `npm audit --audit-level=high` (high-severity `browserslist`, transitively via `@babel`), fixed by a lockfile update in the next commit; counting of fully green runs starts there (Q02 gate: three consecutive) | [Delivery evidence](research/2026-09-20-planning-evidence.md), [DISCOVERIES](../DISCOVERIES.md) |
