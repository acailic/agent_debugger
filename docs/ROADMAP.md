...
| Q06 | PARTIAL→ | Hosted auth hardened: absent key → 401 in cloud mode, analytics routes authenticated in hosted mode (open locally), both inventory gaps closed with matrix tests (`e7f61b2`). Remaining: hosted-startup e2e, checkpoint event/session consistency, tenant-scoped analytics store | — | ✓(auth) |
