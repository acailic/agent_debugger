...
def _find_high_value_indices(
    events: list["TraceEvent"], threshold: float
) -> set[int]:
    """Find indices of events with importance >= threshold."""
    return {
        i for i, event in enumerate(events) if (event.importance or 0) >= threshold
    }


def _protect_context_window(
    high_value_indices: set[int], num_events: int, context_window: int
) -> set[int]:
    """Protect indices around high-value events within context window."""
...
def _find_low_value_runs(
    protected_indices: set[int], num_events: int, min_segment_length: int
) -> list[tuple[int, int]]:
    """Find contiguous runs of unprotected indices."""
...
def identify_low_value_segments(
    ...
    high_value_indices = _find_high_value_indices(events, threshold)
    protected = _protect_context_window(
        high_value_indices, len(events), context_window
    )
    segments = _find_low_value_runs(protected, len(events), min_segment_length)

    return [_build_collapsed_segment(events, start, end) for start, end in segments]
